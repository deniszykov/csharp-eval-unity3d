...
namespace GameDevWare.Dynamic.Expressions.Binding
{
	internal sealed class TypeConversion
	{
		...
		static TypeConversion()
		{
			...
			SetNaturalConversion(QUALITY_IMPLICIT_CONVERSION, typeof(char), typeof(ushort), typeof(uint), typeof(int), typeof(long), typeof(ulong), typeof(float),
				typeof(double));
			SetNaturalConversion(QUALITY_IMPLICIT_CONVERSION, typeof(float), typeof(double));
			SetNaturalConversion(QUALITY_IMPLICIT_CONVERSION, typeof(byte), typeof(ushort), typeof(short), typeof(uint), typeof(int), typeof(long), typeof(ulong),
			...
			SetNaturalConversion(QUALITY_EXPLICIT_CONVERSION, typeof(double), typeof(float), typeof(long), typeof(ulong), typeof(int), typeof(uint), typeof(short),
				typeof(char), typeof(ushort), typeof(sbyte), typeof(byte));
			SetNaturalConversion(QUALITY_EXPLICIT_CONVERSION, typeof(float), typeof(long), typeof(ulong), typeof(int), typeof(uint), typeof(short), typeof(char),
				typeof(ushort), typeof(sbyte), typeof(byte));
			...
			SetNaturalConversion(QUALITY_IMPLICIT_CONVERSION, typeof(ulong), typeof(long), typeof(uint), typeof(int), typeof(char), typeof(ushort), typeof(short),
			...
			SetNaturalConversion(QUALITY_EXPLICIT_CONVERSION, typeof(int), typeof(ulong), typeof(uint), typeof(short), typeof(char), typeof(ushort), typeof(sbyte),
				typeof(byte));
			SetNaturalConversion(QUALITY_EXPLICIT_CONVERSION, typeof(long), typeof(ulong), typeof(int), typeof(uint), typeof(short), typeof(char), typeof(ushort),
		...
		{
			if (typeDescriptions == null) throw new ArgumentNullException(nameof(typeDescriptions));
			...
			lock (Conversions)
			{
				foreach (var typeDescription in typeDescriptions)
				{
					foreach (var conversionMethod in typeDescription.Conversions)
					{
						...
						var explicitConversionMethod = conversionMethod.IsImplicitOperator ? null : conversionMethod;
						var implicitConversionMethod = conversionMethod.IsImplicitOperator ? conversionMethod : null;
		...
		private static void SetNaturalConversion(float quality, Type fromType, params Type[] toTypes)
		{
			...
			{
				var key = new TypeTuple2(fromType, toType);
				Conversions.Add(key, new TypeConversion(quality, true));
			}
		}

		public override string ToString()
		{
			return string.Format("Quality: {0}, Is Natural: {1}, Implicit: {2}, Explicit: {3}", this.Quality.ToString(CultureInfo.InvariantCulture), this.IsNatural.ToString(), this.Implicit,
				this.Explicit);
		}
	}
}
