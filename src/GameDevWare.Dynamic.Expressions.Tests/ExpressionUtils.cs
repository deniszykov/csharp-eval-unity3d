...
public class ExpressionUtils
{
	...
	public static void Execute(string expression, Type[] types, bool forceAot, ITypeResolver typeResolver = null, params object[] arguments)
	{
		...
		var compileMethod = typeof(ExpressionExtensions)
			...
			.GetDeclaredMethods()
			.Where(m => m.IsPublic && m.IsStatic)
			.Single(m => m.Name == "CompileAot" &&
				m.ReturnType.Name.StartsWith("Action") &&
				(types.Length > 0 ? m.IsGenericMethod && m.GetGenericArguments().Length == types.Length : !m.IsGenericMethod));
